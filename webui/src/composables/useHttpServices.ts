...
import type { HttpServiceItem } from '../types/http-service'
...
export function useHttpServices() {
  const route = useRoute()
  const router = useRouter()

  const loading = ref(false)
  const error = ref<string | null>(null)
  const items = ref<HttpServiceItem[]>([])
  ...
  async function load() {
    ...
    try {
      const response = await fetchPaginatedJSON<HttpServiceItem>(`/http/services?${params.toString()}`)
      items.value = response.data
      nextPage.value = response.nextPage
    } catch (err) {
      error.value = err instanceof Error ? err.message : 'Unable to load HTTP services.'
      items.value = []
      nextPage.value = 1
    } finally {
      loading.value = false
