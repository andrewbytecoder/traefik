...
import type { HttpRouterItem } from '../types/http-router'
...
export function useHttpRouters() {
  ...
  const items = ref<HttpRouterItem[]>([])
  ...
  async function load() {
    ...
    try {
      const response = await fetchPaginatedJSON<HttpRouterItem>(`/http/routers?${params.toString()}`)
      items.value = response.data
      nextPage.value = response.nextPage
    } catch (err) {
      error.value = err instanceof Error ? err.message : 'Unable to load HTTP routers.'
      items.value = []
      nextPage.value = 1
    } finally {
      loading.value = false
