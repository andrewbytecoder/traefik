...
const config = getResourceConfig('udp-services')

if (!config) {
  throw new Error('Missing resource config for udp-services')
}
...
const rawPayloadEntries = computed(() => {
  ...
  const hiddenKeys = new Set(['name', 'status', 'provider', 'type', 'loadBalancer', 'weighted', 'usedBy'])
...
<template>
  <div class="detail-shell" data-testid="udp-service-detail-page">
    <div class="detail-header">
      <div>
        <div class="eyebrow">UDP</div>
        <h1 class="detail-title">{{ displayName }}</h1>
        <p class="detail-description">
          Inspect the selected UDP service, including backend targets, weighted services, and routers that
          depend on it.
        </p>
      </div>
    ...
    <v-alert
      ...
      variant="tonal"
      border="start"
      title="Failed to fetch UDP service detail"
      :text="error"
    />

    <template v-if="loading">
      <v-row dense>
        <v-col v-for="index in 3" :key="`udp-service-detail-skeleton-${index}`" cols="12" md="4">
        ...
        <v-col cols="12" md="8">
          <v-card rounded="xl" variant="outlined" class="fill-height">
            ...
            <v-card-text>
              ...
              <div v-else class="empty-copy">No direct backend servers are defined for this UDP service.</div>
            </v-card-text>
          </v-card>
        </v-col>
      </v-row>
          ...
          <v-card rounded="xl" variant="outlined" class="fill-height">
            ...
            <v-card-text>
              <template v-if="usedBy.length">
                <div class="chip-wrap">
                  <router-link v-for="routerName in usedBy" :key="routerName" class="chip-link" :to="`/udp/routers/${encodeURIComponent(routerName)}`">
                    <v-chip size="small" variant="outlined">{{ routerName }}</v-chip>
                  </router-link>
                </div>
              </template>
              <div v-else class="empty-copy">No routers reference this UDP service.</div>
            </v-card-text>
          </v-card>
        </v-col>
      </v-row>
