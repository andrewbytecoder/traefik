...
const config = getResourceConfig('udp-routers')

if (!config) {
  throw new Error('Missing resource config for udp-routers')
}
...
const rawPayloadEntries = computed(() => {
  ...
  const hiddenKeys = new Set(['name', 'status', 'provider', 'service', 'priority', 'priorityStr', 'using', 'entryPoints'])
  return Object.entries(item.value).filter(([key]) => !hiddenKeys.has(key))
})

const detailRouteBase = computed(() => `/udp/routers/${encodeURIComponent(String(route.params.name || ''))}`)
</script>

<template>
  <div class="detail-shell" data-testid="udp-router-detail-page">
    <div class="detail-header">
      <div>
        <div class="eyebrow">UDP</div>
        <h1 class="detail-title">{{ displayName }}</h1>
        <p class="detail-description">
          Inspect the selected UDP router, including the linked service, entrypoints, and raw router
          payload exposed by Traefik.
        </p>
      </div>
    ...
    <v-alert
      ...
      variant="tonal"
      border="start"
      title="Failed to fetch UDP router detail"
      :text="error"
    />

    <template v-if="loading">
      <v-row dense>
        <v-col v-for="index in 3" :key="`udp-router-detail-skeleton-${index}`" cols="12" md="4">
      ...
      <v-row dense>
        ...
        <v-col cols="12" md="8">
          <v-card rounded="xl" variant="outlined" class="fill-height">
            <v-card-title>Routing Summary</v-card-title>
            <v-card-text class="link-grid">
              <div>
                <div class="summary-label">Linked service</div>
                <router-link v-if="item.service" class="name-link" :to="`/udp/services/${encodeURIComponent(String(item.service))}`">
                  {{ item.service }}
                </router-link>
...
.summary-grid, .payload-grid, .linked-list, .link-grid { display: grid; gap: 1rem; }
