...
const config = getResourceConfig('http-services')

if (!config) {
  throw new Error('Missing resource config for http-services')
}
...
const mirrorServices = computed(() => (((item.value?.mirroring as { mirrors?: Array<{ name: string; percent?: number }> })?.mirrors) || []))
...
const rawPayloadEntries = computed(() => {
  ...
  const hiddenKeys = new Set(['name', 'status', 'provider', 'type', 'loadBalancer', 'weighted', 'mirroring', 'usedBy', 'serverStatus'])
...
<template>
  <div class="detail-shell" data-testid="http-service-detail-page">
    <div class="detail-header">
      <div>
        <div class="eyebrow">HTTP</div>
        <h1 class="detail-title">{{ displayName }}</h1>
        <p class="detail-description">
          Inspect the selected HTTP service, including backend servers, health checks, weighted or mirrored
          targets, and routers that depend on the service.
        </p>
      </div>
    ...
    <v-alert
      ...
      variant="tonal"
      border="start"
      title="Failed to fetch HTTP service detail"
      :text="error"
    />

    <template v-if="loading">
      <v-row dense>
        <v-col v-for="index in 3" :key="`http-service-detail-skeleton-${index}`" cols="12" md="4">
    ...
    <template v-else-if="item">
        ...
        <v-col cols="12" md="4">
          <v-card rounded="xl" variant="elevated" class="fill-height">
            ...
            <v-card-text class="summary-grid">
              ...
              <div v-if="loadBalancer" class="summary-item">
                <div class="summary-label">Pass host header</div>
                <div class="summary-value">{{ loadBalancer.passHostHeader ? 'True' : 'False' }}</div>
              </div>
            </v-card-text>
          </v-card>
        </v-col>
        ...
        <v-col cols="12" md="8">
          <v-card rounded="xl" variant="outlined" class="fill-height">
            ...
            <v-card-text>
              ...
              <div v-else class="empty-copy">No direct backend servers are defined for this service.</div>
            </v-card-text>
          </v-card>
        </v-col>
      </v-row>
          ...
          <v-card rounded="xl" variant="outlined" class="fill-height">
            ...
            <v-card-text>
              <template v-if="usedBy.length">
                <div class="chip-wrap">
                  <router-link v-for="routerName in usedBy" :key="routerName" class="chip-link" :to="`/http/routers/${encodeURIComponent(routerName)}`">
                    <v-chip size="small" variant="outlined">{{ routerName }}</v-chip>
                  </router-link>
                </div>
              </template>
              <div v-else class="empty-copy">No routers reference this service.</div>
            </v-card-text>
          </v-card>
        </v-col>
      </v-row>
      ...
      <v-row dense>
        ...
        <v-col cols="12" md="6">
          <v-card rounded="xl" variant="elevated" class="fill-height">
            <v-card-title>Mirrors</v-card-title>
            <v-card-text>
              <template v-if="mirrorServices.length">
                <v-table density="comfortable">
                  <thead>
                    <tr>
                      <th>Service</th>
                  ...
                  <tbody>
                    <tr v-for="mirror in mirrorServices" :key="mirror.name">
                      <td>{{ mirror.name }}</td>
                      <td>{{ mirror.percent ?? '-' }}</td>
                    </tr>
                  </tbody>
                </v-table>
              </template>
              <div v-else class="empty-copy">This service does not define mirror targets.</div>
            </v-card-text>
          </v-card>
        </v-col>
      </v-row>
