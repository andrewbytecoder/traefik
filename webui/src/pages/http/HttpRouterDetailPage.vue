...
const config = getResourceConfig('http-routers')

if (!config) {
  throw new Error('Missing resource config for http-routers')
}
...
const tlsDomains = computed(() => {
  const tls = item.value?.tls as { domains?: Array<{ main: string; sans?: string[] }> } | undefined
  return tls?.domains || []
})
...
const rawPayloadEntries = computed(() => {
  ...
  const hiddenKeys = new Set(['name', 'status', 'provider', 'service', 'priority', 'priorityStr', 'using', 'entryPoints', 'middlewares', 'tls', 'rule', 'error'])
...
const detailRouteBase = computed(() => `/http/routers/${encodeURIComponent(String(route.params.name || ''))}`)
</script>

<template>
  <div class="detail-shell" data-testid="http-router-detail-page">
    <div class="detail-header">
      <div>
        <div class="eyebrow">HTTP</div>
        <h1 class="detail-title">{{ displayName }}</h1>
        <p class="detail-description">
          Inspect the selected HTTP router, including its rule, linked service, entrypoints, middlewares,
          TLS settings, and any routing errors reported by Traefik.
        </p>
      </div>
      <v-chip color="primary" variant="tonal" prepend-icon="mdi-vector-polyline">
        HTTP Router
      </v-chip>
    </div>
    ...
    <v-alert
      ...
      variant="tonal"
      border="start"
      title="Failed to fetch HTTP router detail"
      :text="error"
    />

    <template v-if="loading">
      <v-row dense>
        <v-col v-for="index in 3" :key="`http-router-detail-skeleton-${index}`" cols="12" md="4">
      ...
      <v-row dense>
        ...
        <v-col cols="12" md="8">
          <v-card rounded="xl" variant="outlined" class="fill-height">
            <v-card-title>Routing Rule</v-card-title>
            <v-card-text class="rule-block">{{ item.rule || '-' }}</v-card-text>
            <v-divider />
            <v-card-text class="link-grid">
              <div>
                <div class="summary-label">Linked service</div>
                <router-link v-if="item.service" class="name-link" :to="`/http/services/${encodeURIComponent(String(item.service))}`">
                  {{ item.service }}
                </router-link>
              ...
              <template v-if="middlewareNames.length">
                <div class="chip-wrap">
                  <router-link
                    ...
                    :key="middlewareName"
                    class="chip-link"
                    :to="`/http/middlewares/${encodeURIComponent(middlewareName)}`"
                  >
          ...
          <template v-if="item.tls">
            <div class="tls-grid">
              <div class="summary-item">
                ...
                <div class="summary-value">{{ item.tls.options || '-' }}</div>
              </div>
              <div class="summary-item">
                <div class="summary-label">Cert resolver</div>
                <div class="summary-value">{{ item.tls.certResolver || '-' }}</div>
              </div>
            </div>
            <v-table v-if="tlsDomains.length" class="mt-4" density="comfortable">
              <thead>
                <tr>
                  <th>Main domain</th>
                  <th>SANs</th>
              ...
              <tbody>
                <tr v-for="domain in tlsDomains" :key="domain.main">
                  <td>{{ domain.main }}</td>
                  <td>
                    <div class="chip-wrap">
                      <v-chip v-for="san in domain.sans || []" :key="san" size="x-small" variant="outlined">
                        {{ san }}
                      </v-chip>
                    </div>
                  </td>
