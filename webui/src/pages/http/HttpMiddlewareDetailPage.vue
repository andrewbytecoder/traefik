...
<template>
  <div class="detail-shell" data-testid="http-middleware-detail-page">
    <div class="detail-header">
      <div>
        <div class="eyebrow">HTTP</div>
        <h1 class="detail-title">{{ displayName }}</h1>
        <p class="detail-description">
          Inspect the selected HTTP middleware, including its type, provider, routers that use it, runtime
          errors, and the middleware-specific configuration payload.
        </p>
      </div>
    ...
    <v-alert
      ...
      variant="tonal"
      border="start"
      title="Failed to fetch HTTP middleware detail"
      :text="error"
    />

    <template v-if="loading">
      <v-row dense>
        <v-col v-for="index in 3" :key="`http-middleware-detail-skeleton-${index}`" cols="12" md="4">
